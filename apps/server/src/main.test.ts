...
import type { OrchestrationReadModel } from "@arbortools/contracts";
    ...
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            ARBOR_STATE_DIR: uniqueStateDir,
            ...env,
          },
        }),
      ),
...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect("prefers --mode over ARBOR_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over ARBOR_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        ARBOR_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        ARBOR_MODE: "desktop",
        ARBOR_LOG_WS_EVENTS: "false",
        ARBOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        ARBOR_NO_BROWSER: "true",
      });
