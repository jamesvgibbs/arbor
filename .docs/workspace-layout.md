...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `arbor` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@arbortools/shared/git`, `@arbortools/shared/DrainableWorker`) — no barrel index.
