...
- Publishes the CLI package (`apps/server`, npm package `arbor`) with OIDC trusted publishing.
...
- Repository slug source:
  - `ARBOR_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `ARBOR_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `arbor` (or rename package first if needed).
