...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit ARBOR_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "ARBOR_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid ARBOR_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults state dir to ~/.arbor/dev when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ARBOR_STATE_DIR, defaultStateDir);
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ARBOR_STATE_DIR, resolve("/tmp/override-state"));
        assert.equal(env.ARBOR_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.ARBOR_NO_BROWSER, "1");
        assert.equal(env.ARBOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.ARBOR_LOG_WS_EVENTS, "1");
        assert.equal(env.ARBOR_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            ARBOR_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          stateDir: undefined,
        ...
        assert.equal(env.ARBOR_MODE, "web");
        assert.equal(env.ARBOR_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ARBOR_LOG_WS_EVENTS, "0");
      }),
    );
  });
