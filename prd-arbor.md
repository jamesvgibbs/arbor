...
**Technologies**: Electron, TypeScript, React, Tailwind CSS, `simple-git` (npm), Octokit GitHub SDK, `@anthropic-ai/claude-code` SDK, Turborepo monorepo
...
- The session conversation must be embedded in the Arbor UI (not opened in an external terminal), using the Claude Code SDK/IPC layer
...
- The Electron + Claude Code SDK foundation provides working agent session plumbing out of the box
...
Arbor uses an Electron shell + Node WebSocket server + React web app architecture.
...
[React Renderer Process]
  ...
  └── Session View (Claude Code stream UI)
...
#### 1. [DOMAIN] Establish Arbor Project Identity

1. **Depends On**: Nothing
2. **Description**: Set up the Arbor project, verify the Claude Code adapter builds and runs end-to-end
3. **Implementation**:
   a. Set up `package.json` app name, Electron `productName`, window title
   b. Verify Claude Code adapter is functional; run local smoke test
   c. Establish Arbor-specific directory structure:
...
4. **Acceptance Criteria**:
   a. App launches as "Arbor" with no legacy references in visible UI
   b. A Claude Code session can be started against a manually specified local directory
...
3. **Implementation**:
   ...
   iv. Launch Claude Code session rooted at worktree path
